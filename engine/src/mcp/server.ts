...
/**
 * MCP control plane (docs/plan/02 §MCP surface). Runs over stdio against the
 * same database as the engine:  `pnpm mcp`  (env: DATABASE_URL, CRED_MASTER_KEY).
 * This is where the single human touchpoint — credential handoff — lands.
 */
...
server.tool(
  'register_credential',
  'The human handoff: store network credentials and immediately verify them. ' +
    'Secrets by network — impact: {account_sid, auth_token}; stripe: {api_key, webhook_secret}; ' +
    'amazon (tag-link mode): none needed; csv/direct sources: none needed.',
  {
    network: z.string().describe('impact | stripe | amazon | csv:<slug> | direct:<slug>'),
  ...
  async (args) => {
    ...
    return text({
      ...
      webhook_url: adapter.handleWebhook ? `${config.PUBLIC_BASE_URL}/v1/webhooks/${source.id}` : undefined,
